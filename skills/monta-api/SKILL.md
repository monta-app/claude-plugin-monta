...
- **Dates**: send UTC ISO 8601 with a `Z` suffix (`2026-09-01T00:00:00Z`) for `fromDate`, `toDate`, and `fromUpdatedDate`. Statistics endpoints (`get-site-statistics`, `get-charge-point-statistics`) take `YYYY-MM-DD`; the insight reports (`get-insights-charges-*`) need full timestamps despite their spec. Both allow at most 31 days per request. Timestamps in responses are UTC. Convert to the user's timezone when presenting, and say which timezone you used.
